...
// 22 / 06 / 26
// En BuilderEntidadEstatica, solucione los errores relacionados a ->Add utilizando arreglos dinámicos
generic<typename T>
array<T>^ appendToArray(array<T>^ arr, T item) {
    int oldLen = arr->Length;
    array<T>^ tmp = gcnew array<T>(oldLen + 1);
    for (int i = 0; i < oldLen; i++) tmp[i] = arr[i];
    tmp[oldLen] = item;
    return tmp;
}

EntidadEstatica^ setupNINivel1Mapa1Lenador1() {
    // arraya de opciones 0
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(
        L"Increparlo",
        gcnew array<int>{0, 0, 1},
        1
    ));
    opciones0 = appendToArray(opciones0, gcnew Opcion(
    ...
    // arraya de opciones 1
    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Quería decir algo más...", gcnew array<int>{0, 0, 0}, 0));
...
EntidadEstatica^ setupNINivel1Mapa2Wilmer() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Así que me estaban esperando...", gcnew array<int>{0,0,1}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"¿Los Mudo?", gcnew array<int>{0,0,1}, 2));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"¿Solo una molestia menor? ¿Te estás refiriendo a los anteriores periodistas, no?", gcnew array<int>{0,0,1}, 3));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"[Asentir con la cabeza y no decir nada]", gcnew array<int>{0,0,1}, 4));

    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"[Asentir con la cabeza]", gcnew array<int>{0,0,0}, 4));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Quisiera preguntarte algo más... [Volver]", gcnew array<int>{0,0,0}, 0));
    ...
    array<Opcion^>^ opciones2 = gcnew array<Opcion^>(0);
    opciones2 = appendToArray(opciones2, gcnew Opcion(L"No me dejé convencer. Por eso no me marché. Quiero ser la excepción a la regla.", gcnew array<int>{5,0,0}, 6));
    opciones2 = appendToArray(opciones2, gcnew Opcion(L"Ya lo dijiste. Soy periodista. Vine a hacer mi trabajo, nada más.", gcnew array<int>{0,0,0}, 7));
    opciones2 = appendToArray(opciones2, gcnew Opcion(L"No tengo por qué contarle mis intenciones a nadie.", gcnew array<int>{-5,0,0}, 7));

    array<Opcion^>^ opciones3 = gcnew array<Opcion^>(0);
    opciones3 = appendToArray(opciones3, gcnew Opcion(L"Vengo de estar con los pobladores. He aprendido mucho. [MIN. CONF: 30] [ACABAR NIVEL]", gcnew array<int>{0,0,0}, 7, 30, 0, true));
...
EntidadEstatica^ setupNINivel1Mapa2Kevin() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Se llaman trenes eléctricos. Y quizás se puede decir que vuelan...", gcnew array<int>{0,0,0}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"No, es una exageración. Son trenes peculiares, nada más.", gcnew array<int>{0,0,0}, 2));

    Dialogo^ d0 = gcnew Dialogo(gcnew array<String^>{
    ...
    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Está bien, pero por un momento. Y la cuidas, por favor.", gcnew array<int>{1,0,0}, 3));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Lo siento, es mi herramienta de trabajo. No puedo.", gcnew array<int>{0,0,0}, 4));

    Dialogo^ d1 = gcnew Dialogo(gcnew array<String^>{
    ...
    Dialogo^ d2 = gcnew Dialogo(gcnew array<String^>{
    ...
    array<Opcion^>^ opciones3 = gcnew array<Opcion^>(0);
    opciones3 = appendToArray(opciones3, gcnew Opcion(L"¿Algo más? ¿Qué es lo que pasa?", gcnew array<int>{5,5,5}, 5));

    Dialogo^ d3 = gcnew Dialogo(gcnew array<String^>{
    ...
    array<Opcion^>^ opciones4 = gcnew array<Opcion^>(0);
    opciones4 = appendToArray(opciones4, gcnew Opcion(L"Cambié de opinión. Ten, tómala, pero cúidala mucho, y regresa rápido.", gcnew array<int>{0,0,0}, 3));

    Dialogo^ d4 = gcnew Dialogo(gcnew array<String^>{
    ...
    array<Opcion^>^ opciones5 = gcnew array<Opcion^>(0);
    opciones5 = appendToArray(opciones5, gcnew Opcion(L"¿Le 'pegaron' a Wilmer?", gcnew array<int>{0,0,1}, 6));

    Dialogo^ d5 = gcnew Dialogo(gcnew array<String^>{
    ...
    Dialogo^ d6 = gcnew Dialogo(gcnew array<String^>{
...
EntidadEstatica^ setupNINivel1Mapa2Rosa() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"[Asentir con la cabeza y ayudarla]", gcnew array<int>{5,10,0}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"¿Qué...?", gcnew array<int>{0,0,0}, 2));

    array<Opcion^>^ opciones0_1 = gcnew array<Opcion^>(0);
    opciones0_1 = appendToArray(opciones0_1, gcnew Opcion(L"[Asentir con la cabeza y ayudarla]", gcnew array<int>{5,10,0}, 1));
...
EntidadEstatica^ setupNINivel1Mapa2Tomas() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"[Sentarse a su lado, manteniendo silencio]", gcnew array<int>{2,2,0}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Buenos días, Abuelo... ¿Qué es lo que observa?", gcnew array<int>{0,0,0}, 2));
    ...
    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"[Tomar el tabaco y fumarlo a su lado]", gcnew array<int>{3,3,3}, 3));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"¿Qué es lo que observas, Abuelo?", gcnew array<int>{0,0,0}, 2));
...
EntidadEstatica^ setupNINivel1Mapa3Elena() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Soy periodista. De Crónica Viva. ¿Es usted la maestra?", gcnew array<int>{2,0,0}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"¿Dos forasteros? ¿En seis meses?", gcnew array<int>{2,0,0}, 2));
    ...
    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"¿La historia es diferente? Cuéntame más [MIN 25 DE CONFIANZA]", gcnew array<int>{2,2,0}, 3, 30));
    ...
    array<Opcion^>^ opciones2 = gcnew array<Opcion^>(0);
    opciones2 = appendToArray(opciones2, gcnew Opcion(L"¿Desertaron? ¿Para ir a trabajar?", gcnew array<int>{5,8,0}, 4));
...
EntidadEstatica^ setupNINivel1Mapa3Silvia() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"¿Le diste agua de río? [MIN. CON. LENGUA: 15]", gcnew array<int>{0,5,0}, 1, 5, 15, false));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Tu hijo está enfermo. ¿Puedo verlo? [MIN. CON. LENGUA: 10]", gcnew array<int>{2,5,5}, 2, 5, 10, false));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Eh...", gcnew array<int>{0,0,0}, 0));
    ...
    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Prometo buscar ayuda", gcnew array<int>{6,3,5}, 3));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Llévaselo a la profesora Elena. Y no vuelvas a darle agua del río...", gcnew array<int>{1,3,3}, 4));
...
EntidadEstatica^ setupNINivel1Mapa3Clara() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Buenos días, ¿Es usted la curandera del pueblo?", gcnew array<int>{0,0,0}, 1));

    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Curandera. Es la palabra más adecuada.", gcnew array<int>{7,0,7}, 2));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"¿Mamá Clara? Si no me equivoco, así la llaman en el pueblo", gcnew array<int>{4,0,7}, 3));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"No lo sé. No la conozco bien.", gcnew array<int>{5,0,7}, 4));
    ...
    Dialogo^ d7 = gcnew Dialogo(gcnew array<String^>{
...
EntidadEstatica^ setupNINivel1Mapa4Maza() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Sí, soy periodista. ¿A poco a que me estabas esperando, eh?", gcnew array<int>{0,0,0}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"No tengo por qué decirte quien soy", gcnew array<int>{0,0,0}, 2));
    ...
    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"¿Quieres que te pague para que hables? Va. Así es como funciona el mundo real.", gcnew array<int>{-3,0,5}, 4));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"¿Un incentivo? Lo siento, no he caído tan bajo como para dar sobornos.", gcnew array<int>{3,0,0}, 5));
...
EntidadEstatica^ setupNINivel1Mapa4Jeremias() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Sí, soy de fiar.", gcnew array<int>{1,0,0}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"Eso espero.", gcnew array<int>{0,0,0}, 1));
    ...
    array<Opcion^>^ opciones1 = gcnew array<Opcion^>(0);
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"Cuéntamelo. Estás en discresión conmigo.", gcnew array<int>{3,0,6}, 2));
    opciones1 = appendToArray(opciones1, gcnew Opcion(L"¿Por qué me lo dices a mí, el forastero, y no a tu comunidad?", gcnew array<int>{0,0,6}, 3));
...
EntidadEstatica^ setupNINivel1Mapa4Timoteo() {
    array<Opcion^>^ opciones0 = gcnew array<Opcion^>(0);
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"¿Qué es lo que le pasa al río?", gcnew array<int>{3,1,5}, 1));
    opciones0 = appendToArray(opciones0, gcnew Opcion(L"¿Es por esas manchas brillantes y espumosas?", gcnew array<int>{5,1,5}, 2));
